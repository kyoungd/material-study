...
def FindTrends(df, n: int = 25, distance_factor: float = 0.1, extend_lines: bool = False):
    ...
    df['min'] = df.iloc[argrelextrema(
        df.close.values, np.less_equal, order=n)[0]]['close']
    df['max'] = df.iloc[argrelextrema(
        df.close.values, np.greater_equal, order=n)[0]]['close']
        ...
        for i2, p2 in dfMax.iterrows():
            if i1 + 1 < i2:
                if p1['max'] <= p2['max']:
                    ...
                    # length of trend
                    line_length = np.sqrt((point2[0] - point1[0])
                    ...
                    for i3 in range(i1 + 1, i2):
                        ...
                        if not pd.isna(df.iloc[i3, :]['max']):
                            ...
                            d = la.norm(np.cross(point2-point1, point1-point3)) / \
                                la.norm(
                                    point2-point1)
...
def Main():
    """"""
    filename_only = "4228dab6-e5c1-4ae3-9dc5-6cc601c84c79"
    data = AlpacaDownloader.load_dataframe(filename_only)

    symbol = "GOOG"
    df = data[symbol]
    times = df.index.values
    # for tm in df.index.values:
    #     x = tm.timestamp().value
    times = [pd.Timestamp(tm).value / 1000000 for tm in df.index.values]
    # convert times to timestamp
    df1 = pd.DataFrame({'time': times, 'open': df.open.values,
                       'high': df.high.values, 'low': df.low.values, 'close': df.close.values, 'volume': df.volume.values})
    df1['date'] = pd.to_datetime(
        df1['time'] * 1000000, infer_datetime_format=True)

    # convert dataframe transposed time to colume
    lines = FindTrends(df1, distance_factor=10, n=6, extend_lines=True)
    PlotData(df1, trends=lines, plot_title=symbol+" trends")
